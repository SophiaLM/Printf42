...
/*   Updated: 2024/02/08 19:18:22 by soluna           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print(char const *str, va_list argv, int i, int *count)
{
	...
	if (str[i] == 'u')
		ft_putbase(va_arg(argv, unsigned int), "0123456789", 10, count);
	if (str[i] == 'X')
		ft_putbase(va_arg(argv, unsigned int), "0123456789ABCDEF", 16, count);
	if (str[i] == 'x')
		ft_putbase(va_arg(argv, unsigned int), "0123456789abcdef", 16, count);
	if (str[i] == '%')
		ft_putchar('%', count);
